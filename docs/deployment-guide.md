...
7. [Обход замедления Telegram](#7-обход-замедления-telegram)
8. [Создание и адаптация стратегий](#8-создание-и-адаптация-стратегий)
9. [Перевод Flowseal .bat → zapret2](#9-перевод-flowseal-bat--zapret2)
10. [systemd-сервис](#10-systemd-сервис)
11. [Критические грабли](#11-критические-грабли)
12. [Диагностика](#12-диагностика)
13. [Чеклист после развёртывания](#13-чеклист-после-развёртывания)
...
├── lists/                            # Пользовательские IP-листы (хранятся в git)
│   └── ipset-telegram.txt            # IP-диапазоны Telegram DC1-DC5
...
| `$BASE_DIR` | `/opt/zapret-scripts` | При source (в пользовательских переменных) |
| `$LISTS_DIR` | `/opt/zapret-scripts/zapret-latest/lists` | При eval (в STRATEGY) |

**Пользовательские переменные**: можно определять в стратегии ВНЕ heredoc — они раскрываются при source.
Внутри STRATEGY (heredoc `'EOF'`) такая переменная будет раскрыта через `eval` при запуске.

```bash
# Пример: переменная для своего IP-листа
TELEGRAM_IPSET="$BASE_DIR/lists/ipset-telegram.txt"  # раскрывается при source

read -r -d '' STRATEGY << 'EOF' || true
--filter-tcp=443 --ipset=$TELEGRAM_IPSET ...          # $TELEGRAM_IPSET раскрывается через eval
EOF
```
...
## 7. Обход замедления Telegram

### Как Роскомнадзор замедляет Telegram (с февраля 2026)

ТСПУ (DPI-оборудование на сетях всех провайдеров) идентифицирует и замедляет:
- **MTProto** — основной протокол Telegram (TCP, порты 443, 80, 5222)
- **STUN/UDP** — установка peer-to-peer соединений для голосовых/видеозвонков (UDP 590-1400, 3478)

### Что добавлено в стратегию `flowseal_fake_tls_auto_alt2`

#### 1. Telegram звонки (UDP/STUN)

```bash
--filter-udp=590-1400,3478 --filter-l7=stun --lua-desync=fake:blob=stun:repeats=6
```

Перехватывает STUN-пакеты, которые устанавливают p2p-соединение для звонков.
Инжектирует фейковые STUN-пакеты (`stun.bin`), не позволяя DPI распознать протокол.

> **Важно**: для восстановления качества звонков стратегия должна работать **у обоих участников**.
> Если один звонит через обход, а второй — нет, один из абонентов не слышит другого.

#### 2. Telegram сообщения/MTProto (TCP/443)

```bash
--filter-tcp=443 --filter-l7=tls --ipset=$TELEGRAM_IPSET --payload=tls_client_hello \
...
Фильтрует TCP-соединения к IP-адресам Telegram DC (по `lists/ipset-telegram.txt`).
Применяет `fake + multisplit + seqovl` — та же техника, что работает для YouTube.

IP-диапазоны в `lists/ipset-telegram.txt`:
```
149.154.160.0/20   # DC1-DC5 основной блок
91.108.4.0/22      # DC5 и другие
91.108.8.0/22
91.108.12.0/22
91.108.16.0/22
91.108.56.0/22
...
#### 3. MTProto-прокси (TCP, нестандартный порт)

```bash
--filter-tcp=49312 --lua-desync=fake:blob=fake_default_tls:tcp_seq=10000000:repeats=8:tls_mod=rnd,dupsid,sni=www.google.com \
  --lua-desync=multisplit:pos=1:seqovl=681:seqovl_pattern=tls_google
```

Для случаев когда используется MTProto-прокси на нестандартном порту (например, `v.zozo.gg:49312`).
Нет `--filter-l7` — обрабатывается весь TCP-трафик на этом порту вне зависимости от протокола.

> **Примечание**: MTProto-прокси использует обфускацию, DPI обычно не распознаёт его.
> Правило для порта 49312 — страховка на случай блокировки по характеру соединения.
> Порт прописан в `TCP_PORTS`, что также добавляет его в nftables QUEUE.

### Обновление IP-листа Telegram

Если Telegram добавит новые IP-диапазоны:

```bash
# Отредактировать локально
nano lists/ipset-telegram.txt

# Скопировать на Orange Pi
scp lists/ipset-telegram.txt root@192.168.1.73:/opt/zapret-scripts/lists/

# Перезапустить (nfqws2 перечитает список)
ssh root@192.168.1.73 "systemctl restart zapret_discord_youtube"
```

Официальные IP-диапазоны Telegram: [core.telegram.org/resources/cidr.txt](https://core.telegram.org/resources/cidr.txt)

---

## 8. Создание и адаптация стратегий
...
## 9. Перевод Flowseal .bat → zapret2
...
## 13. Чеклист после развёртывания
...
- [ ] `curl -s --max-time 10 https://web.telegram.org` → HTTP 200
- [ ] YouTube работает из браузера клиента за роутером
- [ ] Discord работает из браузера клиента за роутером
- [ ] Telegram (сообщения + медиа) работают без задержек
- [ ] Telegram звонок — нормальная связь (нужно проверить с обеих сторон)
